  ...
  const videoGenItems: MenuEntry[] = [
    {
      id: 'g-model',
      label: '模型',
      value: MODEL_OPTIONS.find(m => m.id === `m:${pref.model}`)?.label ?? 'MiniMax H3',
      submenu: [
        { id: 'm:MiniMax-H3', label: 'MiniMax H3' },
        {
    ...
    },
    {
      id: 'g-quality',
      label: '清晰度',
      value: pref.quality === '2K' ? '高清 2K' : '标准 768P',
      submenu: QUALITY_OPTIONS.map(o => ({ id: o.id, label: o.label })),
    },
    {
      id: 'g-duration',
      label: '时长',
      value: `${pref.duration} 秒`,
    ...
    },
    {
      id: 'g-optimize',
      label: '优化提示词',
      value: pref.promptOptimize === 'none' ? '不需要优化' : '自动优化',
      submenu: PROMPT_OPTIMIZE_OPTIONS.map(o => ({ id: o.id, label: o.label })),
    },
    {
      id: 'g-input-mode',
      label: '输入模式',
      value: INPUT_MODE_OPTIONS.find(o => o.id === `im:${pref.inputMode}`)?.label ?? '自动识别',
  ...
  const videoPackItems: MenuEntry[] = [
    {
      id: 'g-subtitle',
      label: '字幕样式',
      value: SUBTITLE_OPTIONS.find(o => o.id === `st:${pref.subtitleStyle}`)?.label ?? pref.subtitleStyle,
      submenu: SUBTITLE_OPTIONS.map(o => ({ id: o.id, label: o.label })),
    },
    {
      id: 'g-package',
      label: '包装',
      value: pref.packageEnabled ? '自动包装' : '不自动包装',
      submenu: [
