...
export function InputBar({
  ...
  // HeightLab：模板「做同款」——把同款指令填入输入框并聚焦（聚焦触发
  // 推荐区 Manus 收起）。
  useEffect(() => {
  ...
  // HeightLab：模板「做同款」——直接把同款指令发送出去（不留在输入框）。
  useEffect(() => {
  ...
  // HeightLab：内容模板「从聊天开始」激活选项时只聚焦输入框（不填提示词）。
  useEffect(() => {
    ...
    for (const b of boundaries) {
      ...
      if (b.kind === 'chip') {
        ...
        backdrop.push(
          <mark
            key={`ref-${b.ordinal}`}
            className={clsx(css.textRef, b.ref.trigger === '【' && css.templateRef)}
